-- sql_projet.sql - Livrable 3 complet
...
CREATE TABLE avis(
   ...
   note DECIMAL(3,1),
   PRIMARY KEY(id_avis),
   UNIQUE KEY (utilisateur_id, cable_id),
   FOREIGN KEY(utilisateur_id) REFERENCES utilisateur(id_utilisateur),
   FOREIGN KEY(cable_id) REFERENCES cable(id_cable)
);
...
CREATE TABLE liste_envies(
   ...
   date_ajout DATETIME,
   rang INT,
   PRIMARY KEY(utilisateur_id, cable_id),
   FOREIGN KEY(utilisateur_id) REFERENCES utilisateur(id_utilisateur),
   FOREIGN KEY(cable_id) REFERENCES cable(id_cable)
);
...
INSERT INTO couleur (nom_couleur, code_hex) VALUES
('Unique','#888888'),
('Noir','#000000'),
('Blanc','#FFFFFF'),
('Gris','#808080'),
...
(1,'en attente'),(2,'payée'),(3,'expédiée'),(4,'livrée'),(5,'annulée');
...
-- Déclinaisons : câble_id=1 (HDMI Basic 1m): plusieurs couleurs
...
-- MAJ stock câble (somme des déclinaisons)
UPDATE cable SET stock=(SELECT SUM(d.stock) FROM declinaison_cable d WHERE d.cable_id=cable.id_cable)
WHERE id_cable IN (1,2,3,6,11,16);
...
INSERT INTO ligne_panier (cable_id, utilisateur_id, declinaison_cable_id, quantite_panier, date_ajout) VALUES
(4,2,NULL,1,NOW()),(5,3,NULL,1,NOW());
